%function LBP = calculateLBP(img)
    ...
    %I = rgb2gray(img); %converts the truecolor image RGB to the grayscale image I ( by eliminating the hue and saturation information while retaining the luminance)
    %LBP = extractLBPFeatures(I,'CellSize',[32 32],'Normalization',"None"); % il resto dei parametri è di default
    %LBP = LBP/norm(LBP);
    %LBP = bsxfun(@rdivide,LBP,sum(LBP));
    %histogram(LBP); %Plot discrete sequence data
%-----------------------------------------------------------    
...
function result = calculateLBP(varargin) % image,radius,neighbors,mapping,mode)
% Version 0.3.3
